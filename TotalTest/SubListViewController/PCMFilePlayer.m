...
// 初始化播放器
- (int)player:(NSString *)filePath{
    if (!isCanStart) {
        [self AudioQueueStopIsUpdateUI:NO];
        return -1;
    }
    isCanStart = NO;
...
//        NSLog(@"time = %f %f %f",timeStamp.mSampleTime,audioDescription.mSampleRate,timeStamp.mSampleTime / audioDescription.mSampleRate);
//
//    });
    return 1;
...
// 结束
-(void)AudioQueueStopIsUpdateUI:(BOOL)isUpdate{
    ...
    if (isUpdate) {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"videoStopToUpdateUI" object:nil];
    }
}
...
-(void)readPCMAndPlay:(AudioQueueRef)outQ buffer:(AudioQueueBufferRef)outQB
{
    ...
    if (readLength == 0) {
            //　　这里可以进行一些UI控件的刷新操作
            [self AudioQueueStopIsUpdateUI:YES];
    }
